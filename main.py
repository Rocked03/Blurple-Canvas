...
def dev():
    async def pred(ctx):
        return ctx.author.id in bot.allowedusers
...
async def blacklist(ctx):
    try:
        return 573392328912404480 not in [
            r.id for r in (await bot.blurpleguild.fetch_member(ctx.author.id)).roles
        ]
    except Exception:
        return False
...
async def helpformatter(ctx, command):
    ...
    try:
        ...
        try:
            command.commands
            embed.set_footer(
                text=f"{str(ctx.author)} | {bot.user.name} | Showing commands for '{command.qualified_name}' | To see more info on a command, type {ctx.prefix}help {command.qualified_name} <command>"
            )
        except Exception:
            embed.set_footer(text=f"Showing commands for '{command.qualified_name}'")
    except AttributeError:
        embed.set_author(name=f"Bot commands")
        embed.set_footer(
            text=f"{str(ctx.author)} | {bot.user.name} | To see more info on a command, type {ctx.prefix}help <command>"
        )
    try:
        params = []
        for param, specs in dict(command.clean_params).items():
            ...
            if spec.default == None:
                spec = spec.replace(default=specs.empty)
            params.append(f"<{spec}>")
        paramsstr = " ".join(params)
        commandname = command.emoji_name
        if len(command.aliases) != 0:
            commandname = f'[{command.emoji_name}|{"|".join(command.aliases)}]'
        embed.add_field(
            name="Usage",
            value=f"`{ctx.prefix}{command.full_parent_name.strip()}{spacestrip(command.full_parent_name)}{commandname.strip()} {paramsstr}`",
            inline=False,
        )
    except Exception:
        pass
    ...
    try:
        for subcommand in sorted(command.commands, key=lambda item: item.emoji_name):
            ...
            for param, specs in dict(subcommand.clean_params).items():
                ...
                params.append(f"<{spec}>")
            paramsstr = " ".join(params)
            paramsstr = f"{ctx.prefix}{subcommand.full_parent_name.strip()}{spacestrip(subcommand.full_parent_name)}{subcommand.emoji_name.strip()} {paramsstr}"
            try:
                subsubcommands = []
                for c in sorted(subcommand.commands, key=lambda item: item.emoji_name):
                    if not (c.hidden) and await c.can_run(ctx):
                        subsubcommands.append(c.emoji_name)
            except Exception:
                subsubcommands = None
            if subsubcommands != None and len(subsubcommands) != 0:
                ...
                subsubcommandtxt = ""
            embed.add_field(
                name=subcommand.emoji_name,
                value=f"{desc} \n`{paramsstr}` {subsubcommandtxt}",
                inline=False,
            )
    except Exception as e:
        print(e)
