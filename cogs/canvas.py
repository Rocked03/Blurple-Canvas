import discord, asyncio, datetime, PIL, random, time, aiohttp, io, math, typing, traceback, sys
from discord.ext import commands
from discord.ext.commands.cooldowns import BucketType
from PIL import Image, ImageDraw, ImageFont

def dev():
    async def pred(ctx): return ctx.author.id in ctx.bot.allowedusers
    return commands.check(pred)

def inteam():
    async def pred(ctx): return any(elem in [v for k, v in ctx.bot.teams.items()] for elem in [i.id for i in ctx.author.roles])
    return commands.check(pred)

def mod():
    async def pred(ctx): return any(elem in [v for k, v in ctx.bot.modroles.items()] for elem in [i.id for i in ctx.author.roles])
    return commands.check(pred)
...
class CanvasCog(commands.Cog, name = "Canvas"):
    ...
    class image():
        ...
        def imager(self, aboard, x, y, zoom):
            ...
            size = (int(imagemax - ((imagemax - borderwidth - pixelwidth * (width + 1)) % width)), 
                    int(imagemax - ((imagemax - borderwidth - pixelwidth * (height + 1)) % height)))
            pixelsizex = (list(size)[0] - borderwidth - pixelwidth * (width + 1)) / width
            pixelsizey = (list(size)[1] - borderwidth - pixelwidth * (height + 1)) / height
            ...
            draw.rectangle([(borderwidth + 1, borderwidth + 1), size], fill = (78, 93, 148, 255))
            ...
            for yc, yn in zip(raw, range(zoom)):
                for xc, xn in zip(yc, range(zoom)): 
                    draw.rectangle([
                        (int(borderwidth + pixelwidth * (xn + 1) + pixelsizex * (xn)), 
                         int(borderwidth + pixelwidth * (yn + 1)) + pixelsizey * (yn)),
                        (int(borderwidth + pixelwidth * (xn + 1) + pixelsizex * (xn + 1)), 
                         int(borderwidth + pixelwidth * (yn + 1) + pixelsizey * (yn + 1)))
                        ], fill = self.bot.coloursrgb[xc]) # Pixels

            draw.rectangle([
                (int(borderwidth + pixelwidth * (locx) + pixelsizex * (locx - 1)), 
                 int(borderwidth + pixelwidth * (locy)) + pixelsizey * (locy - 1)),
                (int(borderwidth + pixelwidth * (locx) + pixelsizex * (locx)), 
                 int(borderwidth + pixelwidth * (locy) + pixelsizey * (locy)))
                ], fill = None, outline = (255, 255, 255, 255)) # Location highlight
            ...
            draw.rectangle([
                ...
                (int(borderwidth + pixelwidth * (locx) + pixelsizex * (locx)), 
                 borderwidth)
                ], fill = self.bot.coloursrgb[raw[0][locx - 1]])
            draw.line((
                (int(borderwidth + pixelwidth * (locx) + pixelsizex * (locx - 1)), 
                 int(round(borderwidth - (pixelsizex / 3), 0) - 1)),
                (int(borderwidth + pixelwidth * (locx) + pixelsizex * (locx)), 
                 int(round(borderwidth - (pixelsizex / 3), 0) - 1))
                 ), fill = (78, 93, 148, 255), width = 1) # Highlight x
            ...
            draw.rectangle([
                ...
                (borderwidth,
                 int(borderwidth + pixelwidth * (locy) + pixelsizey * (locy)))
                ], fill = self.bot.coloursrgb[raw[locy - 1][0]])
            draw.line((
                (int(round(borderwidth - (pixelsizey / 3), 0) - 1)),
                 int(borderwidth + pixelwidth * (locy) + pixelsizey * (locy - 1)), 
                (int(round(borderwidth - (pixelsizey / 3), 0) - 1)),
                 int(borderwidth + pixelwidth * (locy) + pixelsizey * (locy))
                ), fill = (78, 93, 148, 255), width = 1) # Hightlight y

            tsx, tsy = self.image.fontxy.getsize(f"{x}  =  x")
            draw.text((sizex - tsx - ((borderwidth - tsy) / 2), 
                      (borderwidth - tsy) / 2), 
                      f"{x}  =  x", font = self.image.fontxy, fill = (185, 196, 237, 255))

            tsx, tsy = self.image.fontxy.getsize(f"y  =  {y}")
            txt = Image.new('RGBA', (tsx, tsy))
            ImageDraw.Draw(txt).text((0, 0), f"y  =  {y}", font = self.image.fontxy, fill = (185, 196, 237, 255))
            ftxt = txt.rotate(90, expand = 1)
            board.paste(ftxt, box = (
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandOnCooldown):
            ...
            if minutes: await ctx.send(f"{ctx.author.mention}, this command is on cooldown ({minutes}m, {seconds}s)")
            else: await ctx.send(f"{ctx.author.mention}, this command is on cooldown ({seconds}s)")
            return
    ...
    def screen(self, board, x: int, y: int, zoom: int = 7):
        ...
        if x < zoom / 2: 
            ...
            locx = zoom - (self.bot.board.width - x)
        ...
        if y < zoom / 2: 
            ...
            locy = zoom - (self.bot.board.height - y)
        ...
        for yn, xs in board.data.items():
            if tly <= yn < tly + zoom:
                ...
                for xn, pixel in xs.items():
                    if tlx <= xn < tlx + zoom:
                        de.append("<:" + self.bot.colours[pixel["c"]] + ">")
                        dr.append(pixel["c"])
                if dr:
                    demoji.append(de)
                    draw.append(dr)
    ...
    class boardspec(commands.Converter):
        async def convert(self, ctx, seed):
            options = ['random']
            if seed.lower() in [i.lower() for i in options]: return seed.lower()
    ...
    async def createboard(self, ctx, x: int, y: int, seed: typing.Optional[boardspec] = None, *, name: str):
        """Creates board"""
        if any(i < 5 for i in [x, y]): return await ctx.send("Please have a minimum of 5.")

        await ctx.send("Creating board...")

        fill = "blank"

        try: self.bot.boards[name.lower()]
        except Exception: pass
        else: return await ctx.send("There's already a board with that name!")

        self.bot.boards[name.lower()] = self.board(name = name, width = x, height = y)
        ...
        for yn in range(1, y + 1):
            ...
            for xn in range(1, x + 1):
                if seed == "random": 
                    newboard.data[yn][xn] = {"c": random.choice([name for name in self.bot.colours.keys() if name not in ['edit', 'blank']]), "info": {"user": "Automatic", "time": datetime.datetime.utcnow()}} 
                else:
                    newboard.data[yn][xn] = {"c": "blank", "info": {"user": "Automatic", "time": datetime.datetime.utcnow()}} 
    ...
    async def join(self, ctx, *, name: str):
        """Joins a board"""
        if name not in self.bot.boards.keys(): return await ctx.send(f'That is not a valid board. To see all valid boards, type `{ctx.prefix}boards`.')
        
        self.bot.uboards[ctx.author.id] = name

        bname = self.bot.boards[name].name
        await ctx.send(f"Joined '{bname}' board")

    async def findboard(self, ctx):
        try: self.bot.uboards[ctx.author.id]
        except KeyError: 
            await ctx.send(f"You haven't joined a board! Type `{ctx.prefix}join` to join a board!")
            return False
    ...
    async def view(self, ctx, *, xyz: coordinates):
        ...
        if board.data == None: return await ctx.send('There is currently no board created')

        if x < 1 or x > board.width or y < 1 or y > board.height:
            return await ctx.send(f'Please send coordinates between (1, 1) and ({board.width}, {board.height})')
        ...
        if zoom < 5: return await ctx.send(f'Please have a minumum zoom of 5 tiles')

        async with aiohttp.ClientSession() as session:
            async with ctx.typing():
                start = time.time()
                image = await self.bot.loop.run_in_executor(None, self.image.imager, self, board, x, y, zoom)
                end = time.time()
                ...
                embed.set_footer(text = f"{str(ctx.author)} - {ctx.prefix}{ctx.command.name}", icon_url = ctx.author.avatar_url)
    ...
    async def viewnav(self, ctx, *, xyz: coordinates):
        ...
        if locx - 2 < 0: display += (self.bot.empty * (locx - 2)) + f" **{x}** (x) ➡"
        elif locx > zoom - 1: display += (self.bot.empty * (locx - 2)) + f"⬅ **{x}** (x)"
        else: display += (self.bot.empty * (locx - 2)) + f"⬅ **{x}** (x) ➡"
        ...
        arrows = ["⬅", "⬆", "⬇", "➡"]
        for emote in arrows: await msg.add_reaction(emote)

        def check(payload):
            return payload.user_id == ctx.author.id and payload.message_id == msg.id and str(payload.emoji) in arrows
    ...
    async def place(self, ctx, *, xyz: coordinates):
        ...
        if x < 1 or x > board.width or y < 1 or y > board.height:
            self.bot.cd.add(ctx.author.id)
            await ctx.send(f'Please send coordinates between (1, 1) and ({board.width}, {board.height})')
            return
        ...
        colours = [emoji for name, emoji in self.bot.colours.items() if name not in ['edit', 'blank']]
        colours.append("✖")
        for emoji in colours:
            await msg.add_reaction(emoji)

        def check(reaction, user):
            return user == ctx.author and reaction.message.id == msg.id and str(reaction.emoji).replace("<:", "").replace(">", "") in colours
        ...
        try:
            ...
            if str(reaction.emoji) == "✖":
                ...
                board.data[y][x] = {"c": colour, "info": {"user": ctx.author.id, "time": datetime.datetime.utcnow()}}
        ...
        # display = "\n".join(["".join(i) for i in emoji])
        # embed.set_field_at(0, name = embed.fields[0].name, value = display)
    ...
    async def debugraw(self, ctx):
        ...
        print(board.data)
        print([[xv["c"] for xv in xk.values()] for xk in [yv for yk, yv in board.data.items()]])
